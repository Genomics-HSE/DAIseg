...
def run_daiseg(lmbd_opt,seq, n_st, idx, start, ar_cover):
    d = MU * L
    A = HMM2.initA(L,RR, lmbd_opt[4]/d, lmbd_opt[3])
    

    
    B_our_mas = np.array([HMM2.initB_arch_cover(MU,L, lmbd_opt, n_st, 0.1+i*0.1) for i in range(10)])
    B_Skov = HMM2.initBwN(L, lmbd_opt[0:3], n_st)
    P=[0.97, 0.03]

    tracts_HMM =  HMM2.get_HMM_tracts(HMM2.viterbi_modified(seq [idx], P, A, B_our_mas, B_Skov, ar_cover))
...
def EM_gaps(seq, lambda_0, n_st, cover):
    return EM2.EM_algorithm_gaps(P, seq, n_st, MU, RR, lambda_0, epsilon, L, int(args.EM_steps), gaps_numbers, cover )
