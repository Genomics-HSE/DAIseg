...
def train_em_normalized(
    O1, O2, L1, L2, init_lmbd, rr, Ti, init_a, max_iter=20, tol=1e-8
):
    M, N = O1.shape
...
def run_batch_em_pipeline(
    ...
    for j_file in json_files_list:
        ...
        # Load ND Mask
        path_nd = os.path.join(
            data["prefix"], data.get("window_callability", {}).get("Nd_1k_genomes", "")
        )
        ...
        if path_nd and os.path.exists(path_nd):
            try:
                ...
                if len(cal_nd_1kG) != N_check:
                    print(
                        f"Warning: ND mask length mismatch in {j_file}. Resizing/Zeroing."
                    )
...
def run_batch_em_pipeline_v2(
    json_files_list, output_combined_file=None, max_iter=15, tol=1e-6, threads=4
):
    """
    V2 Pipeline :
    ...
    for it in range(max_iter):
        ...
        for item in batch_data:
            ...
            log_emissions = hmm.compute_emissions_custom(
                O1, O2, L1, L2, full_lambda_vec
            )
            emit_linear = np.exp(log_emissions)
        ...
        #  Update T and a
        new_T, new_a = update_structural_params_logic(
            total_xi, rr_val, L_val, curr_T, curr_a
        )
    ...
    for item in batch_data:
        ...
        result = hmm.run_hmm(
            O1, O2, cal_1kG, cal_nd_1kG, lmbd=final_full_params, rr=rr_val
        )
        ...
        with open(out_path, "w") as f:
            ...
            for sample_name, tracks in out_dict_new.items():
                for start, end in tracks.get("Archaic", []):
                    f.write(
                        f"{sample_name}\t{jsn['CHROM']}\t{start}\t{end}\t{end - start + 1}\n"
                    )
                    if output_combined_file:
                        all_results.append(
                            {
